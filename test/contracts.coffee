...
test "contracts can be embedded in comments", ->
  ...
  compile = (code) ->
    CoffeeScript.compile code, { contracts: true }

  js_with_contract = compile '''
    id :: (Num) -> Num
    id = (x) -> x
  '''
  js_without_contract = compile '''
    id = (x) -> x
  '''
  ...
  for test in positive_tests
    eq compile(test), js_with_contract

  for test in negative_tests
    eq compile(test), js_without_contract
